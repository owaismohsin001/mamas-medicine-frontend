...
import deepFormat from "./deep";
import summaryFormat from "./summary";
...
  `;
  ...
  // WRAPPER
  return { deep: deepFormat(deepHTML), summary: summaryFormat(summaryHTML) }
}

export async function POST(req) {
  try {
    const { email, childName, parentName, insight: insightJSON, deep } = await req.json();
    ...
    const mailOptions = {
      ...
      to: email,
      subject: `:star2: Soul Snapshot for ${safeChildName} is Ready!`,
      html,
    };
