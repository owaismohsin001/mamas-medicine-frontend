...
export function generateSoulReadingHTML(summary, deep, childName = "Child") {
  // --- Preprocess text for deep reading ---
  function preprocess(text) {
    return text
      .replace(/^#{1,6}\s*/gm, "")
  ...
  const renderList = (block) => {
    ...
    return `<ul class="reading-list">${items}</ul>`;
  };

  const renderKeyTip = (block) => {
    const content = block.replace("💡 Key Tip", "").trim();
    return `
      <div class="key-tip-box">
        <span class="key-tip-label">💡 Key Tip</span>
        <p class="key-tip-text">${content}</p>
      </div>
    `;
  };
  ...
  const renderBlock = (block) => {
    ...
    if (isSectionHeader(block)) {
      return `<h2 class="reading-section-title">${block}</h2>`;
    }

    if (isClusterHeader(block)) {
      return `<h3 class="reading-cluster-title">${block}</h3>`;
    }

    if (isList(block)) return renderList(block);

    return `<p class="reading-p">${block}</p>`;
  };
  ...
  const summaryHTML =
    summaryItems.length > 0
      ...
      <div class="reading-summary-card">
        <h2 class="reading-summary-title">
          🌟 Soul Snapshot — You + ${childName}
        </h2>
        <ul class="summary-list">
          ${summaryItems
            .map((item) => {
              ...
              return `
                <li>
                  <span class="bullet-point">${bullet}</span>
  ...
  const deepHTML = `
    <div class="reading-card reading-deep">
